...
export class DefectManagementService extends BaseDataService {
    private apiUrl = 'api/DefectManagement';
    private oDataUrl = 'odata/DefectManagement';
    ...
    getAllData(pageSortFilterData?: PageSortFilterInfo): Observable<ApiResponse<DefectManagement>> {
        const url = `${this.oDataUrl}`;
        return super.get(url, undefined, pageSortFilterData).pipe(map((result) => {
            return new ApiResponse<DefectManagement>(result);
        }));
    }
    ...
    getDataBySectionId(id: number, pageSortFilterData?: PageSortFilterInfo): Observable<ApiResponse<DefectManagement>> {
        const url = `${this.oDataUrl}(${id})`;
        return super.get(url, undefined, pageSortFilterData).pipe(map((result) => {
          return new ApiResponse<DefectManagement>(result);
        }));
    ...
    setDefectSectionPageSortFilterInfo(pageSortFilterInfo: PageSortFilterInfo): ExpandSelectCountInfo {
        pageSortFilterInfo.expandInfo = <ExpandSelectCountInfo>
          {
            expand: <Record<string, ExpandSelectCountInfo>[]>
              [
                this.expandDefectManagement()
              ]
          };
        return pageSortFilterInfo.expandInfo;
      }

      expandDefectManagement(): Record<string, ExpandSelectCountInfo> {
        return {
          'defectManagement': <ExpandSelectCountInfo>{
            expand: <Record<string, ExpandSelectCountInfo>[]>[
