...
export class ProductLifeCycleStageService extends BaseDataService {

  private oDataUrl = 'odata/ProductLifeCycleStage';
  ...
  ): Observable<ApiResponse<ProductLifeCycleStage>> {
    const url = `${this.oDataUrl}`;
    return super.get(url, undefined, pageSortFilterData).pipe(
      map(result => {
        return new ApiResponse<ProductLifeCycleStage>(result);
      })
    );
  }

  searchByField(field: string, value: string): Observable<ApiResponse<ProductLifeCycleStage>> {
