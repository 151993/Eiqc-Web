...
@Component({
  selector: 'app-fun-parameter',
  templateUrl: './fun-parameter.component.html',
  styleUrls: ['./fun-parameter.component.css']
})
export class FunParameterComponent extends BaseDetailComponent implements OnInit {
  ...
  initForm() {
    ...
    this.formInput = this.formBuilder.group({
      ...
      instrumentType: new FormControl({ value: null, disabled: false }, [
        Validators.required]),
      dataType: new FormControl(true, Validators.required),
  ...
  ngOnInit(): void {
    super.ngOnInit();
    this.funParameterType = TabType.FUN;
    this.getDefaultDataType();
    this.formInput.patchValue({
      calculationPoint: Numbers.TwentyFive
    });
    if (this.detail !== undefined) {
      const funParameterModel = new FunParameterModel(this.detail);
      this.editData(funParameterModel);
  ...
  editData(funParameterModel: FunParameterModel) {
    this.isDataTypeCTQ = funParameterModel.dataTypeId === dataTypes[0].id;
    this.onNormalChange(funParameterModel.normalValue);
    this.formInput.patchValue({
      parameterManagement: funParameterModel.parameterManagement,
      uom: funParameterModel.uom,
      normalValue: funParameterModel.normalValue,
      upperTolerance: funParameterModel.upperTolerance,
      lowerTolerance: funParameterModel.lowerTolerance,
      instrumentType: funParameterModel.instrumentType,
      accuracy: funParameterModel.accuracy,
      dataType: funParameterModel.dataType,
      chartType: funParameterModel.chartType,
      calculationPoint: funParameterModel.calculationPoint
